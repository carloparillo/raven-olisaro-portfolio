...
function renderLiveCarousel() {
  ...
  track.innerHTML = liveCarouselImages.map((src, index) => {
    ...
    const portrait = portraitLivePhotoStems.has(stem);
    const imageStyle = getLiveCarouselImageStyle(src);
    return `
      <figure class="live-carousel-slide${portrait ? " is-portrait" : ""}">
        <img src="${src}" alt="${meta.event} live performance, ${meta.place}, ${meta.year}" loading="${index === 0 ? "eager" : "lazy"}" style="object-fit:${imageStyle.fit};object-position:${imageStyle.position};">
